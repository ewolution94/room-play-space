...
## NEXT UP -- Separate single-room and multi-room, for real (2026-07-30)

**Start here next session -- see `HANDOVER.md`'s "Critical next step" section for the full writeup.** User-reported, high priority: single-room and multi-room are currently the same data structure (every "single room" is secretly a one-room `Floor` in `planner-multi-floors`) and the same route (`/rooms/$roomId` for both), which is why finishing the wizard/from-scratch/from-example flow shows a nonsensical "Back to Overview" button and litters the multi-room floor list with one-room floors. Not started.

- [ ] Design + implement a genuinely separate single-room data store (no `Floor` wrapper) and a separate route (e.g. `/room/$roomId`, singular) -- see `HANDOVER.md` for the proposed approach and the rejected simpler alternative (a flag on `Floor`, explicitly not what the user wants).
- [ ] Rewire `CreateSingleRoomFlow.tsx` (both paths) and `IkeaRoomWizard.tsx`'s finish step to use the new store/route instead of `createFloor`/`/rooms/$roomId`.
- [ ] Confirm with the user: single-room "from example" should show their supplied default-office JSON -- verify `buildDefaultOfficeItems()`/`buildDefaultOfficeOpenings()` (already wired into `single-room-templates.ts`'s "Home Office" entry) is exactly right, and ask whether they want the 6-item gallery kept or want "from example" to skip straight to the office demo like the pre-dashboard `/` used to.
- [ ] Confirm with the user: multi-room "from example" should load their supplied apartment-floor JSON -- verify `generateDefaultApartmentLayout()` (`default-apartment.ts`) already reflects it correctly, or get the actual file if not.
- [ ] Re-verify multi-room "from scratch" (empty floor + existing add-room/add-hallway sidebar) still works unchanged after the above -- should be unaffected, but confirm.
