...
- [ ] **Still open, needs your call**: `@lovable.dev/vite-tanstack-config` (the wrapped vite config -- see `vite.config.ts`'s own "do not add manually" comment) unconditionally sets `server.watch.awaitWriteFinish = { stabilityThreshold: 1000, pollInterval: 100 }` for local (non-sandbox) dev specifically -- confirmed via `node_modules/@lovable.dev/vite-tanstack-config/dist/index.js`'s `applyWatchDebounceDefaults()`, and empirically measured a ~1s+ delay between saving a file and Vite's own "page reload"/"hmr update" log line for it. This is very likely a real contributor to "stuck for a long time" during active editing (separate from the build-size issue above, which doesn't affect this). The wrapper's own documented escape hatch (`defineConfig({ vite: {...} })`) can't actually override this -- the hardcoded values always win in the merge regardless of what's passed. Removing it would require intercepting the wrapper's *resolved* config in `vite.config.ts` rather than using its supported extension point, which feels like it's worth explicit confirmation before doing, since the debounce may exist for a legitimate local-filesystem/editor reason that predates this investigation.
- [ ] **Unrelated, low-priority cleanup**: `ai` and `@ai-sdk/openai-compatible` are in `package.json` `dependencies` but have zero imports anywhere in the repo (confirmed via full-repo grep) -- dead weight in `node_modules`/install time, not part of the shipped bundle (nothing imports them, so they were never actually bundled). Safe to remove whenever, just flagging since dependency removal wasn't asked for.
